# Пакет из книги Тома Кайта "Oracle для профессионалов, 3 издание"
# Cобирает статистику для сравнения выполнения двух разных методов решения задачи
# Использование
# 1. exec runstats_pkg.rs_start
# 2. выполнить 1й метод решения задачи
# 3. exec  runstats_pkg.rs_middle
# 4. выполнить 2й метод решения задачи
# 5. exec  runstats_pkg.rs_stop
# Последняя процедура выведет результаты сравнения двух методов.



conn sys/oracle as sysdba

grant select on v_$statname to roiot;
grant select on v_$mystat to roiot;
grant select on v_$latch to roiot;
grant select on v_$timer to roiot;
